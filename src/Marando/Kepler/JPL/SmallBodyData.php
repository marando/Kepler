  ...
  protected function searchComet($name) {
    ...
    while ($file->valid()) {
      ...
      if (strstr(strtolower($cname), strtolower($name))) {
        ...
        $w     = Angle::deg((float)substr($line, 85, 9));
        $node  = Angle::deg((float)substr($line, 95, 9));
        ...
        $tp   = AstroDate::parse("{$tpY}-{$tpM}-$tpD")->add(Time::days($tpDF));

        return Orbitals::comet($epoch, $q, $e, $i, $w, $node, $tp, $cname);
      }
    }
  }
...
/**
       *  $this->updateSmallBodyJPL();

           * 1. Check last update date via http://ssd.jpl.nasa.gov/?sb_elem
           *      preg_match this -> (The files above were updated on 2015-Nov-23 02:32 -0800.)
           *      -- only ever update once every 24 hours even if site says otherwise
           * 2. Download if needed:
           *    http://ssd.jpl.nasa.gov/dat/ELEMENTS.COMET comets
           *    http://ssd.jpl.nasa.gov/dat/ELEMENTS.NUMBR numbered asteroids
           *    http://ssd.jpl.nasa.gov/dat/ELEMENTS.UNNUM un-numbered asteroids
           * 3. flag last pull date
           *
           * - store files using the method in iers to avoid top level dir
           *

       */
      /**
       protected function updateSmallBodyJPL() {
          // Find last updated date
          $p = '/.*The files above were updated on \n*\s*([0-9]{4})-'
                  . '([a-zA-Z]{3})-([0-9]{1,2})\s([0-9]{2}):([0-9]{2})\s-([0-9]{4})/';
          if (preg_match($p, file_get_contents('http://ssd.jpl.nasa.gov/?sb_elem'), $d)) {
            $year  = $d[1];
            $month = $d[2];
            $day   = $d[3];
            $hour  = $d[4];
            ...
            $lastUpdateJPL = AstroDate::parse("$year-$month-$day $hour:$min UT-$utc");
          }

          $comet = file_get_contents('http://ssd.jpl.nasa.gov/dat/ELEMENTS.COMET');
          file_put_contents('comet', $comet);
        }
